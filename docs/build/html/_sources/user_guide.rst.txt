...
* **Curvilign length** : Segmental length of the trajectory, in µm.
* **Duration** : Duration, in seconds, of the trajectory.
* **Fraction of time paused** : Fraction of the time that the feature spent paused.
* **Variance in GO and STOP phases** : Variance of the intensity of the signal in GO and STOP phases.
* 
...
* **Directionality** : Ratio of anterograde to retrograde transport. 1 means a purely retrograde transport, 0 a purely anterograde transport.
* **Phase directionality** : Ratio of the number of retrograde GO phases to the overall number of GO phases.
* **Switch** : Reversals of directionality, as in a GO phase of a given direction followed by a STOP phase then a GO phase of the opposite direction.
* **Normalized switch** : Reversals of directionality per second.
* **Direction-specific switch** : Reversals of directionality separated into anterograde to retrograde, and retrograde to anterograde.
...
* Conditions to be compared are, for now, simply determined by folder structure. 
  ...
  * For unidirectional transport, folder structure is as such : ``input_folder/experiment/condition/well/files.tif``
