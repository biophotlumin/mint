...
def variables_antero_retro(data,input_folder):
    ...
    pure_WT = data.loc[data['condition']=='DYNAPYRAZOLE']
    ...
    for item in set(voi):
        if normality(data,item) == False: #Check for normality
            results.append('Distribution of '+str(item)+' is not normal \n')
            results.append("p-value of Kruskal-Wallis test for "+item+" is "+str(round((kruskal(data,item)),12))+"\n")
            dunn(data,item)
            boxplot(data,item,input_folder,str(round((kruskal(data,item)),6)))
...
def kruskal(data,variable):
    ...
    """
    list_of_arrays = []
    for index in set(data.slide.unique()):
        list_of_arrays.append(data.loc[data.slide==index, variable])
...
def boxplot(data,variable,input_folder,p):
    ...
    plt.savefig(Path(input_folder).joinpath("Boxplot "+str(data['slide'].unique())+" "+variable+".png"))
    plt.close()
...
def barplot(data,variable,input_folder,p):
    ...
    error = []
    for i in set(data['slide'].unique()):
        error.append(stats.sem(data[variable].loc[data['slide']==i],nan_policy='omit'))
    sns.barplot(x=data['slide'],y=data[variable],capsize=0.02,estimator=mean,yerr=error,ci=None)
    sns.despine(trim=True)
    plt.xlabel("Condition")
    plt.ylabel(ylabels[variable])
    plt.annotate(("p-value : "+p),xy=(195,310),xycoords='figure points')
    plt.savefig(Path(input_folder).joinpath("Barplot "+str(data['slide'].unique())+" "+variable+".png"))
    plt.close()
...
def pub_barplot(data, variable,input_folder,p):
    ...
    colorpal = colorpal_kif
    error = []
    error.append(stats.sem(data[variable].loc[data['slide']=='oeil_gauche'],nan_policy='omit'))
    error.append(stats.sem(data[variable].loc[data['slide']=='oeil_droit'],nan_policy='omit'))
    if variable == 'run_length_retro' or variable == 'curvilign_velocity_retro':
        sns.barplot(y=(data[variable]*-1),x=data['slide'],estimator=mean,yerr=error,ci=None,\
            error_kw={'elinewidth':2,'capsize':4,'capthick':2})
    else:
        sns.barplot(y=data[variable],x=data['slide'],estimator=mean,yerr=error,ci=None,\
            error_kw={'elinewidth':2,'capsize':4,'capthick':2})
    sns.despine(trim=True)
...
if __name__ == '__main__':
    input_folder = r'/media/baptiste/SHG_tracking_data/Zebrafish data/Dyna_tri_complet Results - 20220413_114440 0.64/left right'
