...
def variables(data,input_folder):
    """Calls statistical tests and plotting functions for each variable of interest. Inputs a DataFrame and a string.
        ...
        input_folder is the path to the input folder.
    """
    voi = ['curvilign_velocity','proc','run_length','diag_size',\
    ...
    for item in set(voi):
        if normality(data,item) == False: #Check for normality
            ...
            results.append("p-value of Wilcoxon rank sum test for "+item+" is "+str(round((ranksums(data,item)),6))+"\n")
            boxplot(data,item,input_folder,str(round((ranksums(data,item)),6)))
            barplot(data,item,input_folder,str(round((ranksums(data,item)),6)))
...
if __name__ == '__main__':
    input_folder = r'/media/baptiste/SHG_tracking_data/Louison_Lallemant/data extraction Results - 20220112_175107'
    settings = {'antero_retro':False}
