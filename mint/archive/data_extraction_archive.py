...
# from parallel import p_phase_calculations
...
def per_traj(data, trajectory, settings, parameters, condition, slide, name, animal):
    ...
    if settings['polynomial_fit']: # Fit trajectories to third-degree polynom, and discard trajectories that deviate too much
        if polynomial_fit(subdata,parameters['len_cutoff'],parameters['threshold_poly3']) == True:
            ...
            return # pd.DataFrame({'col': [False]}) ## Got to have a reliable way of logging rejections
...
def phase_calculations(parameters,data,settings,condition,slide,name,animal):
    ...
    for trajectory in set(data.particle):
        ...
        if settings['polynomial_fit']: # Fit trajectories to third-degree polynom, and discard trajectories that deviate too much
            if polynomial_fit(subdata,parameters['len_cutoff'],parameters['threshold_poly3']) == True:
                pass
            elif polynomial_fit(subdata,parameters['len_cutoff'],parameters['threshold_poly3']) == 'len':
...
def trajectory_calculations(phase_parameters,parameters,settings):
    ...
    intensity_GO,intensity_STOP,variance_GO,variance_STOP,curvilign_velocity,processivity,run_length,pausing_frequency,pausing_time,\
        diag_size,fraction_paused,moving_particles,moving_particles_msd,number_stop,duration,curv_length,pausing_time,gfp = ([] for _ in range(18))
    ...
    for file in set(phase_parameters.file.unique()):
        ...
        for item in set(phase_parameters[(phase_parameters.file==file)].rejoined_trajectory):
            ...
            # Curvilign size
            curv_size = np.sum(np.abs(data.run_length))
            ...
            # Trajectory duration
            t_duration = (np.sum(data.phase_duration)*parameters['dt'])
            ...
            moving_particles.append((phase_parameters[(phase_parameters.file==file)].rejoined_trajectory.nunique()/np.unique(n_particles))[0]
                                    if np.unique(n_particles)[0] > 0 else 1)
            ...
            moving_particles_msd.append((phase_parameters[(phase_parameters.file==file)].rejoined_trajectory.nunique()/np.unique(n_static))[0]
                                        if np.unique(n_static)[0] > 0 else 1)
            ...
            if settings['antero_retro']:
                ...
                for p in set(data.trajectory): # Useless
                        ...
                        for i in range(len(p_data.phase_number)-2):
                            if (p_data[p_data.phase_number==i].phase.values==2) & (p_data[p_data.phase_number==i+1].phase.values==0) & (p_data[p_data.phase_number==i+2].phase.values==2):
                                if (p_data[p_data.phase_number==i].curvilign_velocity.values > 0) & (p_data[p_data.phase_number==i+2].curvilign_velocity.values <0):
                                    ...
                                    t_pausing_time_switch.append(float(p_data[p_data.phase_number==i+1].phase_duration.values[0]))
                                    ...
                                    t_pausing_time_antero.append(float(p_data[p_data.phase_number==i+1].phase_duration.values[0]))
                                elif (p_data[p_data.phase_number==i].curvilign_velocity.values < 0) & (p_data[p_data.phase_number==i+2].curvilign_velocity.values < 0):
                                    t_pausing_time_retro.append(float(p_data[p_data.phase_number==i+1].phase_duration.values[0]))
...
def phase_calculations_joblib(parameters,data,settings,condition,slide,name,animal):
    ...
    phase_calc_gen = Parallel(n_jobs=os.cpu_count(),return_as='generator')(delayed(per_traj)\
...
def phase_generator(data, trajectory, settings, parameters, condition, slide, name, animal):
...
def temp_wrap(parameters, data, settings, condition, slide, name, animal):
    ...
    for trajectory in set(data.particle):
        subdata = data[data.particle==trajectory]
        subdata = subdata.reset_index(drop = True)
        traj_data = per_traj(data, trajectory, settings, parameters, condition, slide, name, animal)
        f_phase_parameters.reset_index(inplace=True, drop=True)
...
def data_extraction(input_folder,parameters,settings):
    ...
    trajectory_parameters = trajectory_calculations(phase_parameters,parameters,settings)
...
if __name__ == '__main__':
    ...
    start = time.time()
    input_folder = Path(r"/media/lumin/DATA/DATA_DEVRIM Results - 20240104_165320/DATA_DEVRIM")
