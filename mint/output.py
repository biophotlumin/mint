...
def generate_report(input_folder):
    ...
    pdf.cell(0,10,f"Python version : {sys.version} | Conda environment : {sys.executable.split('/')[-3]}", align='C')
    pdf.ln()
    pdf.cell(0,10,f"Trackpy version {tp.__version__} | NumPy version {np.__version__} | CVXPY version {cvxpy.__version__} | SCS version {scs.__version__}", align='C')
    pdf.ln()
    pdf.cell(0,10,f"Input folder :  {log['input_folder']}",align='C')
    pdf.ln()
    pdf.cell(0,10,f"Started : {start_date} at {start_time} | Report generated : {str(datetime.now().strftime('%Y/%m/%d at %Hh%M'))} | Duration : {log['duration']}",align='C')
    pdf.ln(15)
    pdf.set_font('Helvetica','B', size=12)
    pdf.cell(45,10,f'Parameters',align='C')
    pdf.cell(60,10,f'Settings',align='C')
    pdf.ln(10)
    pdf.set_font('Helvetica', size=8)
    line_height = pdf.font_size * 2.5
    ...
    pdf.cell(None,None,f"Number of files analyzed : {log['n_files']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of files retained in tracking: {log['n_a_file']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of files retained in analysis: {log['n_t_file']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of trajectories found : {log['n_traj']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of trajectories rejoined : {str(log['n_rejoined'])}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of trajectories rejected by polynomial fit : {log['r_poly']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of trajectories rejected by abnormal speed : {log['r_speed']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of trajectories retained for analysis : {log['n_a_traj']}", align='L',new_y="NEXT",new_x="LEFT")
    pdf.cell(None,None,f"Number of conditions : {n_cond}", align='L',new_y="NEXT",new_x="LEFT")

    for i in order:
        sub_x = pdf.get_x()
        pdf.ln(5)
        pdf.set_xy(sub_x,pdf.get_y())
        pdf.cell(None,None,f"Number of animals analyzed for condition {i} : {log['n_a_animal_'+i]}", align='L',new_y="NEXT",new_x="LEFT")
        pdf.cell(None,None,f"Number of files analyzed for condition {i} : {log['n_a_file_'+i]}", align='L',new_y="NEXT",new_x="LEFT")
        pdf.cell(None,None,f"Number of trajectories analyzed for condition {i} : {log['n_a_traj_'+i]}", align='L',new_y="NEXT",new_x="LEFT")
        ...
        for l in lines:
            ...
            if lt in vars:
                if list_var:
                    ...
                    page_break +=1
                list_var = []
                list_var.append(l.strip(''))
            else:
                list_var.append(l.strip(''))
        ind_page(pdf,list_var,input_folder,order,parameters,settings,n_cond)

    pdf_path = Path(input_folder).joinpath(str(datetime.now().strftime('%Y%m%d_%H%M%S'))+'_Experiment_report.pdf')
    pdf.output(pdf_path)
