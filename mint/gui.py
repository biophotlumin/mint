"""Provides a graphical user interface (GUI) as an alternative to CLI or script.py

    The GUI essentially recapitulates the parameters and settings available in script.py.
    It is possible to start an entire run, or run the tracking, data extraction and statistical analysis separately.
    Folder and file structure still need to conform to the required format.
    The GUI itself will (for now) freeze until the run is completed.
"""
...
tab_settings = [            
            ...
            [sg.Frame(layout=[
            [sg.Checkbox('MSD', size=(5,1), key='MSD',enable_events=True,default=default_dict['MSD']),\
            ...
            [sg.Text('Spatial Threshold'),sg.InputText(default_text=default_dict['threshold_r'],key='threshold_r', size=(5,1))]],\
                        title='',title_color='white',relief = sg.RELIEF_SUNKEN)],
            [sg.Frame(layout=[  
            [sg.Checkbox('Stub filtering', size=(15,1), key='stub_filtering',enable_events=True,default=default_dict['stub_filtering']),\
                sg.Text('Stub length'), sg.InputText(default_text=default_dict['stub_length'],key='stub_length', disabled=not default_dict['stub_filtering'],size=(5,1))]],\
                    title='',title_color='white',relief = sg.RELIEF_SUNKEN)],
            [sg.Frame(layout=[  
            [sg.Checkbox('SNR estimation', size=(15,1), key='SNR_estimation',enable_events=True,default=default_dict['SNR_estimation']),\
...
tab_extraction = [
            ...
            [sg.Frame(layout=[  
                [sg.Checkbox('Anterograde / retrograde', size=(30,1), key='antero_retro',enable_events=True,default=default_dict['antero_retro'])]],
                    title='Anterograde / retrograde',title_color='white',relief = sg.RELIEF_SUNKEN)],

            [sg.Frame(layout=[  
                [sg.Checkbox('Polynomial fit', size=(15,1), key='polynomial_fit',enable_events=True,default=default_dict['polynomial_fit'])],\
            ...
            [sg.Frame(layout=[  
                [sg.Checkbox('Minimization', size=(15,1), key='minimization',enable_events=True,default=default_dict['minimization']),\
...
tab_stats = [
            [sg.Frame(layout=[  
                [sg.Checkbox('Individual images', size=(20,1), key='individual_images',enable_events=True,default=default_dict['individual_images'])],
                    [sg.Checkbox('Individual text files', size=(25,1), key='individual_txt',enable_events=True,default=default_dict['individual_txt'])],
...
layout = [  
        ...
        [sg.Input(default_text=default_dict['input_folder'],key='input_folder'), sg.FolderBrowse()],
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if event == 'Run statistical analysis':
            start = time.time()
